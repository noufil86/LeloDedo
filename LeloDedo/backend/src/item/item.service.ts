...
export class ItemService {
  ...
  async findAll(owner_id?: number) {
    ...
    if (owner_id) {
      items = await this.repo.find({ 
        where: { owner: { user_id: owner_id } },
        relations: ['owner', 'category'],
      });
    } else {
      items = await this.repo.find({ relations: ['owner', 'category'] });
    }
    return items;
  }
  ...
  async update(id: number, dto: UpdateItemDto, user_id: number) {
    ...
    // Convert availability_status to uppercase for enum
    if (dto.availability_status) {
      dto.availability_status = dto.availability_status.toUpperCase();
    }
