...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        setLoading(true);
        const response = await api.user.getProfile(user.userId);
        if (response.data) {
          ...
          <div className="glass-effect rounded-2xl p-8 border border-border">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              <div className="flex-1">
                <h2 className="text-2xl font-display font-bold mb-1">{profileData?.name || "User"}</h2>
                <p className="text-muted-foreground mb-3">{profileData?.email || "No email"}</p>
                <div className="flex flex-wrap gap-2">
                  ...
                  {profileData?.role !== "admin" && (
                    <div className="px-3 py-1 bg-secondary rounded-full text-sm font-medium flex items-center gap-1">
                      ...
                      {profileData?.rating ? parseFloat(String(profileData.rating)).toFixed(1) : "0"} Rating
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xl font-display font-bold">
                    {profileData?.joinedDate ? new Date(profileData.joinedDate).toLocaleDateString("en-US", { month: "short", year: "numeric" }) : "N/A"}
                  </p>
                </div>
            ...
            <div className="glass-effect rounded-2xl p-6 border border-border">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xl font-display font-bold">{profileData?.totalRentals || 0}</p>
                </div>
